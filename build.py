...
ANALYTICS = (
    ...
    # Lightweight self-hosted consent banner -> flips analytics_storage on accept
    '''<script>(function(){var K="cc-consent",c=null;try{c=localStorage.getItem(K)}catch(e){}function upd(v){gtag("consent","update",{analytics_storage:v?"granted":"denied"})}if(c==="granted"){upd(true);return}if(c==="denied"){return}function ready(){var d=document,b=d.createElement("div");b.setAttribute("role","dialog");b.setAttribute("aria-label","Privacy consent");b.style.cssText="position:fixed;left:0;right:0;bottom:0;z-index:99999;background:#1d1d1d;color:#eee;font:14px/1.5 system-ui,sans-serif;padding:14px 16px;display:flex;flex-wrap:wrap;gap:10px;align-items:center;justify-content:center;box-shadow:0 -2px 10px rgba(0,0,0,.35)";b.innerHTML="<span style='flex:1 1 300px'>We use privacy-friendly analytics to measure traffic. Allow analytics? See our <a style='color:#9cf' href='/privacy'>privacy policy</a>.</span>";function mk(t,bg,fg){var x=d.createElement("button");x.textContent=t;x.style.cssText="cursor:pointer;border:0;border-radius:5px;padding:8px 18px;font:600 14px system-ui;background:"+bg+";color:"+fg;return x}var a=mk("Accept","#6abf69","#10210e"),r=mk("Reject","#555","#eee");a.onclick=function(){try{localStorage.setItem(K,"granted")}catch(e){}upd(true);b.remove()};r.onclick=function(){try{localStorage.setItem(K,"denied")}catch(e){}upd(false);b.remove()};b.appendChild(a);b.appendChild(r);d.body.appendChild(b)}if(document.readyState!=="loading"){ready()}else{document.addEventListener("DOMContentLoaded",ready)}})();</script>'''
)
